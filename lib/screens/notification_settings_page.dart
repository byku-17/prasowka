      ...
      body: ListView(
        children: [
          ...
          if (settings.notificationsEnabled) ...[
            ...
            const Divider(),
            const SectionHeader('RODZAJE ALERTÓW'),
            CheckboxListTile(
              secondary: const Icon(Icons.star_outline),
              title: const Text('Ważne wiadomości'),
              subtitle: const Text('Artykuły pasujące do Twoich zainteresowań i polubień'),
              value: settings.isAlertTypeEnabled(SettingsProvider.alertTypeImportant),
              onChanged: (val) => settings.toggleAlertType(SettingsProvider.alertTypeImportant, val ?? false),
              activeColor: AppTheme.accentFor(context),
            ),
            CheckboxListTile(
              secondary: const Icon(Icons.article_outlined),
              title: const Text('Nowe artykuły'),
              subtitle: const Text('Wszystkie nowe artykuły z czołowych źródeł'),
              value: settings.isAlertTypeEnabled(SettingsProvider.alertTypeNew),
              onChanged: (val) => settings.toggleAlertType(SettingsProvider.alertTypeNew, val ?? false),
              activeColor: AppTheme.accentFor(context),
            ),
            CheckboxListTile(
              secondary: const Icon(Icons.summarize_outlined),
              title: const Text('Podsumowanie dnia'),
              subtitle: const Text('Raz dziennie — przegląd najważniejszych tematów'),
              value: settings.isAlertTypeEnabled(SettingsProvider.alertTypeSummary),
              onChanged: (val) => settings.toggleAlertType(SettingsProvider.alertTypeSummary, val ?? false),
              activeColor: AppTheme.accentFor(context),
            ),
          ],
          const Divider(),
